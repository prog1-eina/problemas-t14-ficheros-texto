 ...
 * Post: Asigna al parámetro «apariciones» el número de veces que el carácter
 *       «caracter» aparece en «flujo».
 */
void contarApariciones(istream &flujo, const char caracter,
    ...
    while (flujo.get(caracterLeido)) {
        // Si el último intento de lectura tuvo éxito, procesa el carácter
        // leído:
...
/*
 * Pre:  ---
 * Post: Si se puede leer el fichero de nombre «nombreFichero», asigna al
 *       parámetro «apariciones» el número de veces que el carácter «caracter»
 *       aparece en el fichero y al parámetro «lecturaOk» el valor «true».
 *       En caso contrario, asigna al parámetro «lecturaOk» el valor «false».
 */
...
int main() {
    ...
    if (lecturaOk) {
        cout << "El carácter '" << caracter << "' aparece " << apariciones
             << " veces." << endl;
        return 0;
    } else {
        cerr << "No ha podido leerse el fichero \"" << nombreFichero << "\"."
             << endl;
        return 1;
    }
}
